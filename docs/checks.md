...
|-------|----------|------------|--------------|-------------|
| Allowed Characters | `RunFilenameChecks` | `filename_characters` | Yes | Ensures filename only contains `[a-zA-Z0-9\-\.]`. |
| Disallowed Sequences | `RunFilenameChecks` | `filename_sequences` | Yes | Detects invalid patterns like `.-.`, `..`, or multiple dots/dashes. |
| Missing Year | `RunFilenameChecks` | `filename_year_missing` | Yes | Ensures movies have a release year in the filename. |
| Redundant Year | `RunFilenameChecks` | `filename_year_redundant` | Yes | Checks if a year tag is redundant (e.g., when using S20YY style naming for episodes). |
| Streaming Service | `RunFilenameChecks` | `filename_streaming` | Yes | Ensures a streaming service tag is present for WEB sources. |
| TV Specials | `RunFilenameChecks` | `filename_tv_special` | Yes | Requires air date and episode title for TV specials (Season 00). |
| Name Mismatch | `RunFilenameChecks` | `filename_generation_mismatch` | Yes | Verifies that the filename matches the name generated from its metadata. |

## Technical Quality Checks (MediaInfo)

| Check | Function | Identifier | Configurable | Description |
|-------|----------|------------|--------------|-------------|
| Video Track Presence | `RunMediaInfoChecks` | `mediainfo_no_video` | No | Verifies that the file contains at least one video track. |
| Interlaced WEB | `RunMediaInfoChecks` | `mediainfo_interlaced_web` | Yes | Issues a warning if a WEB source is detected as interlaced. |
...
| Check | Function | Identifier | Configurable | Description |
|-------|----------|------------|--------------|-------------|
| Matroska Format | `RunMatroskaChecks` | `matroska_ebml_error` | No | Verifies that the file is a valid Matroska (MKV) container. |
...
| Check | Function | Identifier | Configurable | Description |
|-------|----------|------------|--------------|-------------|
| MDB Error | `RunMdbChecks` | `mdb_error` | No | Reports errors when communicating with TMDB/TVDB. |
| No Match Found | `RunMdbChecks` | `mdb_no_match` | No | Issued when the media cannot be found in the database. |
