...
- **Font attachment filenames** — `matroska_font_filename_compliance` is reported by `check` but not auto-fixed. Renaming a font attachment means extracting it, renaming the file, and replacing it with `mkvpropedit --replace-attachment`, which is out of scope for now.
