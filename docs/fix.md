...
- **Re-encoding required (audio)** — `matroska_truehd_compatibility` (a TrueHD track needs a lossy compatibility track muxed alongside it) and `mediainfo_stereo_lossless` (mono/stereo lossless audio should be FLAC) both require an actual encoder, which `fix` does not have; it only edits container metadata and remuxes existing streams.
